...
def main(files_met, files_flux, ds_NEE_ITCpz_met, ds_NEE_ITCpz_flux,  ofname1, ofname2, ofname3, ofname4, plot_dir):
    ...
    nsample = 10
    """
    for m,f in zip(ds_NEE_ITCpz_met, ds_NEE_ITCpz_flux):
        ds_NEE_ITCpz_met = open_file (m)
        ds_NEE_ITCpz_flux = open_file (f)   
    """   
    ...
    for m,f in zip(files_met, files_flux):
        ...
        if len(ds_met) > ((365*time)*(3/4)) and (len(ds_met[ds_met.Tair_qc < 1])/len(ds_met))>0.5:
            ...
            if 'NEE' in ds_flux:                                    
                ...
                """            
                minimum_tair = math.floor(min(ds_NEE_ITCpz_met.Tair))
                maximum_tair = math.ceil(max(ds_NEE_ITCpz_met.Tair))
                bins = np.arange(minimum_tair+1, maximum_tair + binsize, binsize)
                bin_label = np.arange(minimum_tair+1,
                                      maximum_tair, binsize)
                data_binned = pd.cut(ds_NEE_ITCpz_met.Tair, bins, labels=bin_label)
                ds_NEE_ITCpz_met.loc[:,"tempbin"] = data_binned
                ds_NEE_ITCpz_flux.loc[:, "tempbin"] = data_binned
                ds_NEE_ITCpz_met = ds_NEE_ITCpz_met[ds_NEE_ITCpz_met.Tair_qc<1]
                ds_NEE_ITCpz_flux = ds_NEE_ITCpz_flux[ds_NEE_ITCpz_flux.NEE_qc<1]
         
                # Ordering the data according to the temperature bin, making a
                # pandaframe and equalizing the index to the index of the temperature dataframe
                ds_met_measuredtemp_sorted = ds_NEE_ITCpz_met.groupby(['tempbin']).size()
                df_met_measuredtemp_sorted = pd.DataFrame(ds_met_measuredtemp_sorted , columns = ['measured temp'])
                ds_flux_measuredNEE_sorted = ds_NEE_ITCpz_flux.groupby(['tempbin']).size()
                df_flux_measuredNEE_sorted = pd.DataFrame(ds_flux_measuredNEE_sorted, columns = ['measured NEE'])
                
                dftemp_sorted_NEE = pd.concat([df_met_measuredtemp_sorted, df_flux_measuredNEE_sorted], axis=1)
                #measuredNEE = dftemp_sorted_NEE.iloc[:,0]
                
                # adding the measured NEE to temperature pandas dataframe and the
                # fraction of NEE measurements
                dftemp_sorted.loc[:,"measuredNEE"] = dftemp_sorted_NEE['measured NEE']
                dftemp_sorted.loc[:,"measuredtemp_NEE"] = dftemp_sorted_NEE['measured temp']
                dftemp_sorted.loc[:,'ratioNEE'] = dftemp_sorted_NEE['measured NEE']/\
                                                dftemp_sorted.loc[:,"measuredtemp_NEE"]
                """                                    
            ...
            # This part is to check energy closure. This dit not show any effect
            # on my results
            ...
            if 'NEE' in ds_flux:
                ...
                ratioNEE_overall_mean = 0.54
                #ratioNEE_overall_mean = (dftemp_sorted.measuredNEE.sum()/
                                     #dftemp_sorted_NEE['measured temp'].sum()).round(2)
            ...
            if len(largerthan2_stdev_mean) > 1:
                ...
                if 'NEE' in ds_flux:
                    ...
                    ratioNEE_largerthan2_stdev_mean = 0.56
                    #ratioNEE_largerthan2_stdev_mean = (largerthan2_stdev_mean["measuredNEE"].sum()/
                                                   #largerthan2_stdev_mean["measuredtemp_NEE"].sum()).round(2)
            else:
            ...
            if len(smallerthan2_stdev_mean)>1:
                ...
                if 'NEE' in ds_flux:
                    ...
                    #ratioNEE_smallerthan2_stdev_mean = (smallerthan2_stdev_mean["measuredNEE"].sum()/
                                                   #smallerthan2_stdev_mean["measuredtemp_NEE"].sum()).round(2)
                    ratioNEE_smallerthan2_stdev_mean = 0.33
            else:
            ...
            results_LH.append([mname, lat, lon, mean_tair_measured,  ppt_yearly,
                              ...
                              ratioLH_largerthan2_stdev_mean, color1, length])
            results_SH.append([mname, lat, lon, mean_tair_measured, ppt_yearly,
                              ...
                              ratioSH_largerthan2_stdev_mean, color1,  length])
            results_NEE.append([mname, lat, lon, mean_tair_measured, ppt_yearly,
                              ...
                              ratioNEE_largerthan2_stdev_mean, color1, length])
